...
namespace MP3Info
{
    ...
    public class MP3HeaderReader
    {
        ...
        private string strLengthFormatted;
        ...
        public MP3Header ReadMP3Information(string FileName)
        {
            MP3Header header = new MP3Header();

            FileStream fs = new FileStream(FileName, FileMode.Open, FileAccess.Read);
            // Set the filename not including the path information
            header.FileName = @fs.Name;
            char[] chrSeparators = new char[] { '\\', '/' };
            string[] strSeparator = header.FileName.Split(chrSeparators);
            int intUpper = strSeparator.GetUpperBound(0);
            header.FileName = strSeparator[intUpper];
            ...
            if (fs.Position != fs.Length)
            {
                ...
                header.BitRate = getBitrate(header.FileSize);
                header.Frequency = getFrequency();
                header.Mode = getMode();
                header.Length = new TimeSpan(0, 0, getLengthInSeconds(header.FileSize));
        ...
        private int getBitrate(long fileSize)
        {
            ...
            if (boolVBitRate)
            {
                double medFrameSize = (double)fileSize / (double)getNumberOfFrames(fileSize);
        ...
        private int getLengthInSeconds(long fileSize)
        {
            ...
            return (int)(intKiloBitFileSize / getBitrate(fileSize));
        }

        private int getNumberOfFrames(long fileSize)
        {
            // Again, the number of MPEG frames is dependant on whether it's a variable bitrate MP3 or not
            if (!boolVBitRate)
            {
                double medFrameSize = (double)(((getLayerIndex() == 3) ? 12 : 144) * ((1000.0 * (float)getBitrate(fileSize)) / (float)getFrequency()));
